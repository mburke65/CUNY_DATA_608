...
pay_dataset =  pd.read_csv("https://raw.githubusercontent.com/mburke65/CUNY_DATA_608/master/Final%20Project/MB_Final/raw_data/charting_dataset.csv", low_memory = False)
scatter_dataset = pd.read_csv("https://raw.githubusercontent.com/mburke65/CUNY_DATA_608/master/Final%20Project/MB_Final/raw_data/scatter_dataset.csv", low_memory = False)




available_indicators = pay_dataset['Agency Name'].unique()
calcs_indicators = {'Work Location Borough': ['sum'],'Employee_Id':['sum'], 'Title Description':['sum'],
        'Regular Hours': ['sum'],'Regular Gross Paid':['sum'], 'OT Hours':['sum'],'Total OT Paid':['sum'],
        'Total Other Pay':['sum'], 'Total Pay':['sum']}
...
def generate_table(dataframe, max_rows=10):
    return html.Table(
        ...
        ]) for i in range(min(len(dataframe), max_rows))], style={'font-size':'1.2rem'}
    )
...
def update_scatter(input_value):
	...
	df_plot = scatter_dataset[scatter_dataset['Agency Name'].isin(input_value)]
...
def update_reg_table(input_value):
	df_plot = scatter_dataset[scatter_dataset['Agency Name'].isin(input_value)]
	df_plot= df_plot[['Agency Name', 'Regular Hours','Regular Gross Paid', 'Employee_Id']]
	df_plot = df_plot.loc[df_plot['Regular Hours'] > 0 ]
	df_plot['Regular Gross Paid'] = df_plot['Regular Gross Paid'].map('${:,.2f}'.format)
	df_plot[['Regular Hours','Employee_Id']] =df_plot[['Regular Hours','Employee_Id']].applymap('{:,.1f}'.format)
	return generate_table(df_plot)
...
def update_overtime_table(input_value):
	...
	df_plot = scatter_dataset[scatter_dataset['Agency Name'].isin(input_value)]
	df_plot= df_plot[['Agency Name', 'OT Hours','Total OT Paid', 'Employee_Id']]
	df_plot = df_plot.loc[df_plot['OT Hours'] > 0 ]
	
	df_plot['Total OT Paid'] = df_plot['Total OT Paid'].map('${:,.2f}'.format)
	df_plot[['OT Hours','Employee_Id']] =df_plot[['OT Hours','Employee_Id']].applymap('{:,.1f}'.format)
